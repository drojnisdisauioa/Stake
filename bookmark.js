(() => {
  ...
  const getCur = t => (t.match(/([A-Z]{2,5})$/)?.[1] || '').toLowerCase();

  const convertAll = () => {
    const input = document.querySelector('input[data-test="input-game-amount"]');
    const val = input ? Math.max(0, +input.value) || null : null;

    document.querySelectorAll(CONV_SELECTOR).forEach(div => {
      if (!originalTexts.has(div)) originalTexts.set(div, div.textContent);
      const cur = getCur(div.textContent), price = prices[cur];
      div.textContent = val && price ? (val / price).toFixed(8) + ' ' + cur.toUpperCase() : originalTexts.get(div);
    });
  };

  const hookInput = i => {
    if (!i || i.dataset.hooked) return;
    i.dataset.hooked = '1';
  ...
  const replaceARS = () => {
    const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
      acceptNode: n => n.nodeValue.includes('ARS') ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT
    });
  ...
  const observeTextNodes = () => {
    const observer = new MutationObserver(muts => {
      muts.forEach(m => {
        if (m.type === 'characterData' && m.target.nodeValue.includes('ARS')) {
          m.target.nodeValue = m.target.nodeValue.replace(/ARS[\s\u00A0]*/g, '$');
        }
      });
    });

    const recursiveObserve = node => {
      if (node.nodeType === Node.TEXT_NODE && node.nodeValue.includes('ARS')) {
        observer.observe(node, { characterData: true });
        node.nodeValue = node.nodeValue.replace(/ARS[\s\u00A0]*/g, '$');
      } else if (node.nodeType === Node.ELEMENT_NODE) {
        for (let child of node.childNodes) recursiveObserve(child);
      }
    };

    recursiveObserve(document.body);
    observer.observe(document.body, { childList: true, subtree: true, characterData: true });
  };

  const replacements = [
    {
      current: { fill: "#FFC800", d: "M48 96c26.51 0 48-21.49 48-48S74.51 0 48 0 0 21.49 0 48s21.49 48 48 48" },
      new: { fill: "#6CDE07", d: "M48 96c26.51 0 48-21.49 48-48S74.51 0 48 0 0 21.49 0 48s21.49 48 48 48" }
    },
    {
      current: { fill: "#276304", d: "M79.2 67.32v-4.56l.04.04c5.52-1 8.64-4.88 8.64-10.16 0-6.6-5.56-8.64-9.72-10.16-2.84-1.04-4.68-1.92-4.68-3.68 0-1.48 1.08-2.6 3.32-2.6s4.84.84 6.88 2.68l3.6-5.88c-2.16-1.88-4.96-3.12-8.08-3.56v-4.56h-5.12v4.64c-5.64.96-8.72 5.12-8.72 9.68 0 6.657 5.28 8.558 9.427 10.05l.413.15c2.72 1.04 4.64 1.96 4.64 3.92 0 1.6-1.4 2.84-3.76 2.84-3.12 0-6-1.44-7.92-3.48l-3.76 6.08c2.4 2.32 5.48 3.76 9.68 4.16v4.4z" },
      new: { fill: "#1B3802", d: "M51.52 73.32v6.56h-5.8V73.4c-7.56-.6-13.08-3.56-16.92-7.64l4.72-6.56c2.84 3 6.96 5.68 12.2 6.48V51.64c-7.48-1.88-15.4-4.64-15.4-14.12 0-7.4 6.04-13.32 15.4-14.12v-6.68h5.8v6.84c5.96.6 10.84 2.92 14.6 6.56l-4.88 6.32c-2.68-2.68-6.12-4.36-9.76-5.08v12.52c7.56 2.04 15.72 4.88 15.72 14.6 0 7.4-4.8 13.8-15.72 14.84zm-5.8-30.96V31.04c-4.16.44-6.68 2.68-6.68 5.96 0 2.84 2.84 4.28 6.68 5.36M58.6 59.28c0-3.36-3-4.88-7.04-6.12v12.52c5-.72 7.04-3.64 7.04-6.4" }
    }
  ];

  const deleteAttrs = {
    fill: "#276304", "fill-rule": "evenodd",
    d: "m27.8 62.4-1.24-5.08H16.52l-1.24 5.08H7.16l9.64-32.6h9.52l9.64 32.6zm-6.2-25.68-3.48 13.8h6.96zM53.36 62.4l-4.32-11.24h-2.92V62.4H38.2V29.8h13.28c6.36 0 10.4 4.6 10.4 10.6 0 5.52-2.84 8.32-5.28 9.4l5.52 12.6zm-3.08-25.8h-4.16v7.76h4.16c2.12 0 3.6-1.52 3.6-3.88s-1.52-3.92-3.6-3.92z",
    "clip-rule": "evenodd"
  };
  ...
  const replacePaths = () => {
    document.querySelectorAll('path').forEach(path => {
      for (const { current, new: n } of replacements) {
        if (matches(path, current)) {
          path.setAttribute('fill', n.fill);
          path.setAttribute('d', n.d);
          return;
        }
      }
      if (matches(path, deleteAttrs)) path.remove();
    });
  };
      ...
      muts.forEach(m => {
        m.addedNodes.forEach(n => {
          if (n.nodeType === 1) {
            ...
            n.querySelectorAll && n.querySelectorAll('input[data-test="input-game-amount"]').forEach(hookInput);
          }
        });
      });
      replaceARS();
